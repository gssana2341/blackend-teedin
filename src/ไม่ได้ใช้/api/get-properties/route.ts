    ...
    const [propertiesResponse, detailsResponse, agentsResponse, usersResponse] =
      (await Promise.all([
        supabase
          .from("properties")
          .select(
            "id, agent_id, listing_type, property_category, in_project, rental_duration, location, created_at"
          )
          .limit(500),
        supabase
          .from("property_details")
          .select(
            "property_id, project_name, address, usable_area, bedrooms, bathrooms, parking_spaces, highlight, description, facilities, project_facilities, price, images, latitude, longitude, view_count, created_at"
          )
          .limit(2000),
        supabase
          .from("agens")
          ...
          .limit(1000),
        supabase.from("users").select("id, first_name, last_name").limit(2000),
      ])) as [
